         ...
         for (const auto& bxBlock : bxBlocks) {
            ...
            if (nWords_ <= bxPayload.size()) {
               for (unsigned nWord = 0; nWord < nWords_; nWord += 2) {
                  ...
                  LogDebug("L1T") << "Mu" << nWord/2 << ": eta " << mu.hwEta() << " phi " << mu.hwPhi() << " pT " << mu.hwPt() << " qual " << mu.hwQual() << " sign " << mu.hwSign() << " sign valid " << mu.hwSignValid() << " unconstrained pT " << mu.hwPtUnconstrained();
